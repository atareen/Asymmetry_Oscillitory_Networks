	...
	public NeutralEvolutionApp(boolean activatorTest)
	{
		...
		for(int time=0;time<evolutionTimeTotal;time++)
		{
			...
			if(time==evolutionTimeTotal-1)
			{
				...
				// A1 -> 0, A2 -> 1
				// D1 -> 2, D2 -> 3
				// k_D1_A1 -> NE.keff2D[0][2]
				// k_D1_A2 -> NE.keff2D[1][2]
				
				// k_D2_A1 -> NE.keff2D[0][3]
				// k_D2_A2 -> NE.keff2D[1][3]
				
				
				// Ranjan test
				// set both rate constants to the average of the two
					
				// average(k_A1_D_1, k_A2_D1)
				double average1 = (NE.keff2D[2][0]+NE.keff2D[2][1])/2.0;
				double average2 = (NE.keff2D[3][0]+NE.keff2D[3][1])/2.0;
				
				// replace larger k by average for D1-A
				if(NE.keff2D[2][0]<=NE.keff2D[2][1])
				{
					NE.keff2D[2][0] = average1;
				}
				else
				{
					NE.keff2D[2][1] = average1;
				}
				
				//replace larger k by average for D2-A
				if(NE.keff2D[3][0]<= NE.keff2D[3][1])
				{
					NE.keff2D[3][0] = average2;
				}
				else
				{
					NE.keff2D[3][1] = average2;
				}
				...
				/*
					...
					// set k_D1_A_i to max of both
					if(NE.keff2D[0][2]>=NE.keff2D[1][2])
					{
						NE.keff2D[1][2] = NE.keff2D[0][2]; 
					}
					else
					{
						NE.keff2D[0][2] = NE.keff2D[1][2];
					}
					
					// set k_D2_A_i to max of both
					if(NE.keff2D[0][3]>=NE.keff2D[1][3])
					{
						NE.keff2D[1][3] = NE.keff2D[0][3]; 
					}
					else
					{
						NE.keff2D[0][3] = NE.keff2D[1][3];
					}
					*/
