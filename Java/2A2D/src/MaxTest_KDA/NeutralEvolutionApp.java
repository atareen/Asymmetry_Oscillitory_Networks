		...
		for(int time=0;time<evolutionTimeTotal;time++)
		{
			...
			if(time==evolutionTimeTotal-1)
			{
				...
				// kad test
					...
					// max(k_D1_A_1, k_D2_A1) 
					if(NE.keff2D[0][2]>=NE.keff2D[0][3])
					{
						NE.keff2D[0][3] = NE.keff2D[0][2]; 
					}
					else
					{
						NE.keff2D[0][2] = NE.keff2D[0][3];
					}
					
					// max(k_D1_A_2, k_D2_A2)
					if(NE.keff2D[1][2]>=NE.keff2D[1][3])
					{
						NE.keff2D[1][3] = NE.keff2D[1][2]; 
					}
					else
					{
						NE.keff2D[1][2] = NE.keff2D[1][3];
					}
