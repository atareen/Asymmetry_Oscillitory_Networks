	...
	public NeutralEvolutionApp(boolean activatorTest)
	{
		...
		for(int time=0;time<evolutionTimeTotal;time++)
		{
			...
			if(time==evolutionTimeTotal-1)
			{
				...
				// average(k_D1_A_1, k_D2_A1)
				double average1 = (NE.keff2D[0][2]+NE.keff2D[0][3])/2.0;
				double average2 = (NE.keff2D[1][2]+NE.keff2D[1][3])/2.0;
				
				// replace smaller k by average for D1	
				if(NE.keff2D[0][3]<=NE.keff2D[0][2])
				{
					NE.keff2D[0][3] = average1;
				}
				else
				{
					NE.keff2D[0][2] = average1;
				}
				
				// replace smaller k by average for D2
				if(NE.keff2D[1][3]<=NE.keff2D[1][2])
				{
					NE.keff2D[1][3] = average2;
				}
				else
				{
					NE.keff2D[1][2] = average2;
				}
