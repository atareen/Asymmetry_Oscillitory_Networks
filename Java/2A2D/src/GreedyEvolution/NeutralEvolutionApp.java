	...
	public static void main(String[] args)
	{
		...
		//while(1>0)
		{
			...
			else if(pvrTest[0]-pvrTest[1]<0.5 & pvrTest[2]-pvrTest[3]< 0.5 & pvrTest[4]-pvrTest[5]<0.5 & pvrTest[6]-pvrTest[7]< 0.5)
			{
				...
				// 0 means choose activator fitness, 1 means choose deactivator fitness
				//Random proteinSpecies = new Random();
				// choose random int between 0 and 1
				//protein = proteinSpecies.nextInt(2);
				newFitness = ((Math.abs(PVR[0]-PVR[1]))/(PVR[0]+PVR[1])+(Math.abs(PVR[2]-PVR[3]))/(PVR[2]+PVR[3]))/2.0;
				/*
				if(protein==activator)
				{
					newFitness = (Math.abs(PVR[0]-PVR[1]))/(PVR[0]+PVR[1]);
					//System.out.println("Choosing Activator");
				}
				else if(protein==deactivator)
				{
					newFitness = (Math.abs(PVR[2]-PVR[3]))/(PVR[2]+PVR[3]);
					//System.out.println("Choosing Deactivator");
				}
				*/
				//System.out.println(" Current fitness: "+fitness+ " New fitness: "+newFitness);
