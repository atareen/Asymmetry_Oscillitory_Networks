//10000 0.638936106389361 2K1P
//10000 0.7847215278472153 1K2P
...
public class NeutralEvolutionApp {
	...
	private static double[] maxValue(double[][] solution, int steps,int numberIterations,int N, boolean recordPeak) 
	{
		...
		// start the max at 25000th point in the array so the initial condition doesn't get counted as the peak or valley.
		for(int i=0;i<N;i++)
		{
			max[i]=solution[i][25000];
		}
		for(int j=0;j<N;j++)
		{
			for (int i = 25000; i < numberIterations*steps; i++) 
			{
	...
	public static void main(String[] args) throws FileNotFoundException, UnsupportedEncodingException
	{
		...
		for(int time=0;time<evolutionTimeTotal;time++)
		{
			...
			if(moveAcceptedINT+moveRejected!=0 & iterations%100==0)
			{
				...
				System.out.println(iterations+" "+(moveAcceptedINT)/(1.0*moveAcceptedINT+1.0*moveRejected)+" "+NE.keff2D[2][2]+" "+NE.keff2D[1][0]);
			}
			iterations++;
